...
function getProposal(section, id, value) {
	if (id === 'MODEL') {
		return Object.keys(DATA.skins).filter(v => v.includes(value) && v !== value);
	} else if (id === 'SKIN') {
		return (DATA.skins[section.MODEL] || []).filter(v => v.includes(value) && v !== value);
	} else if (id === 'TRACK') {
		return DATA.tracks.filter(v => v.includes(value) && v !== value);
	} else if (id === 'CONFIG_TRACK' && DATA.layouts[section.TRACK]) {
		return DATA.layouts[section.TRACK].filter(v => v.includes(value) && v !== value);
	}
	return [];
}
	...
	return (
		<div className={`mb-3 ${styles.wrapper}`}>
			...
			<ul className={`${styles.suggestions}`}>
				{dirty &&
					getProposal(props.config[props.section], props.id, value).map(proposal => (
						<li>
							<a
