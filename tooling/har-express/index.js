...
function filter(HAR, req, options = DEFAULT_OPTIONS) {
	...
	if (entries.length > 1) {
		...
		const withSameQueryString = entries.filter((e) => {
			return e.request.queryString.every(
				(qs) => req.params[qs.name] === qs.value
			)
		})
...
function getMiddleware(path, options) {
	...
	return function harMiddleware(req, res, next) {
		...
		if (found.length > 0) {
			...
			res
				.type(candidate.content.mimeType)
				.send(Buffer.from(candidate.content.text, candidate.content.encoding))
		} else {
			next()
		}
	}
